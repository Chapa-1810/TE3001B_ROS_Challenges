...
rclcpp::Publisher<std_msgs::msg::Float32>::SharedPtr publisher_params_;
size_t time_;
...
void timer_callback()
{
  ...
  if (type == 1)
    message.data = amplitude * sin(2 * M_PI * frequency * (time_ + time)/10) + offset;
  else if (type == 2) // Create Square signal
    message.data = amplitude * sin(2 * M_PI * frequency * (time_ + time)/10) > 0 ? amplitude + offset : -amplitude + offset;
  else if (type == 3) // Create Sawtooth signal
    message.data = (fmod((time_ + time) * frequency, 1) * 2 * amplitude) + offset;
  else 
    message.data = 0;
...
{ 
  ...
  publisher_params_ = node_->create_publisher<std_msgs::msg::Float32>("signal_params", 10);
